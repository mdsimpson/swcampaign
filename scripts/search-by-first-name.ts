// Search for residents by first name in production
...
async function searchByFirstName(firstName: string) {
    const query = `
...
async function main() {
    ...
    if (!firstName) {
        console.error('Usage: tsx scripts/search-by-first-name.ts "<firstName>"')
        console.error('Example: tsx scripts/search-by-first-name.ts "Gopi"')
        process.exit(1)
    }

    console.log(`🔍 Searching for residents with first name containing "${firstName}" in production...\n`)

    try {
        const residents = await searchByFirstName(firstName)

        if (residents.length === 0) {
            console.log(`❌ No residents found with first name containing "${firstName}"`)
            process.exit(0)
        }
