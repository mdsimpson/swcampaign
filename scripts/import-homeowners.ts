...
function normalizePhone(s?: string){ if(!s) return undefined; const d=s.replace(/\D/g,''); if(d.length===10) return `(${d.slice(0,3)}) ${d.slice(3,6)}-${d.slice(6)}`; return s.trim() }
async function main(){
  const csvPath = process.argv[2]; if(!csvPath){ console.error('Usage: npm run import:homeowners -- /path/to/Homeowners.csv'); process.exit(1) }
  ...
  for (const row of records){
    ...
    if (p1f || p1l){ await client.models.Person.create({ homeId: home.id, role:'PRIMARY_OWNER', firstName:p1f, lastName:p1l, email: row['Owner Email'] || row['Email'] || undefined, mobilePhone: normalizePhone(row['Cell Phone'] || row['Mobile Phone'] || row['Unit Phone']) } as any); persons++ }
    ...
    if (p2f || p2l){ await client.models.Person.create({ homeId: home.id, role:'SECONDARY_OWNER', firstName:p2f, lastName:p2l, email: row['Secondary Email'] || undefined, mobilePhone: normalizePhone(row['Secondary Cell'] || undefined) } as any); persons++ }
    const rf = row['Renter First Name'] || row['Tenant First Name']
    const rl = row['Renter Last Name'] || row['Tenant Last Name']
    if (rf || rl){ await client.models.Person.create({ homeId: home.id, role:'RENTER', firstName:rf, lastName:rl, email: row['Renter Email'] || undefined, mobilePhone: normalizePhone(row['Renter Cell'] || undefined) } as any); persons++ }
  }
