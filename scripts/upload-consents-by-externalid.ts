// Upload consents from matched_resident_ids.csv using externalId matching
...
async function main() {
    ...
    const rows: ConsentRow[] = parse(csvContent, {
        columns: true,
    ...
    for (const row of rows) {
        const externalId = row.person_id
        ...
        try {
            ...
            if (residents.length === 0) {
                notFound++
                errors.push(`ID ${externalId}: Not found`)
            } else {
